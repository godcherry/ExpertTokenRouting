...
If you are directly using the model downloaded from Huggingface, please make sure you are using our "Qwen/qwen_7b_chat" Huggingface model (rather than "Qwen/Qwen-7B") when you call model.chat().
我们检测到您可能在使用预训练模型（而非chat模型）进行多轮chat，因为您当前在generation_config指定的chat_format，并未设置为我们在对话中所支持的"chatml"格式。
如果您在直接使用我们从Huggingface提供的模型，请确保您在调用model.chat()时，使用的是"Qwen/qwen_7b_chat"模型（而非"Qwen/Qwen-7B"预训练模型）。
"""
...
class QWenPreTrainedModel(PreTrainedModel):
    ...
    def __init__(self, *inputs, **kwargs):
        ...
        self.extra_start = kwargs.get('extra_start', 151646)   # <|extra_0|>
        self.extra_end = kwargs.get('extra_end', 151651)   # <|extra_5|>
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.extra_vocab_size = self.extra_end - self.extra_start + 1
        self.extra_lm_head = nn.Linear(config.hidden_size, self.extra_vocab_size, bias=False)

        if config.bf16:
            self.transformer.bfloat16()
            self.lm_head.bfloat16()
            self.extra_lm_head.bfloat16()
        if config.fp16:
            self.transformer.half()
            self.lm_head.half()
            self.extra_lm_head.half()
        self.post_init()
    ...
    def forward(
        ...
        extra_lm_logit = self.extra_lm_head(hidden_states)

        lm_logits[:, :, self.extra_start: self.extra_end + 1] = extra_lm_logit
